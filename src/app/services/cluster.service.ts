...
import { BehaviorSubject, catchError, filter, firstValueFrom, lastValueFrom, Observable, of, take, tap } from 'rxjs';
...
export class ClusterService {
 ...
 autoClusterListSubject$ = new BehaviorSubject<string[]>([]);
  ...
  totalRows$ = this.totalRowsSubject$.asObservable(); 


  totalRows: number = 0;


  async getAutoClusterData(currentTabKey?: string, params?: { [key: string]: string | number }) {
    try {
      // Always make sure to pass currentTabKey in params!
      const allParams = { ...params, currentTabKey: currentTabKey ? currentTabKey : '' };
      const res = await firstValueFrom(
        this.#clusterApiService.getAutoClusterData(allParams).pipe(
          take(1),
          tap(({ pagedData, totalRows }) => {
            this.autoClusterListSubject$.next(pagedData);
            this.totalRows = totalRows;
            this.totalRowsSubject$.next(this.totalRows);
          }),
          catchError(err => {
            console.error('Error fetching auto cluster data:', err);
            return of({ pagedData: {}, totalRows: 0 });
          })
        )
      );
      return res;
      ...
      return { pagedData: {}, totalRows: 0 };
    }
  }

  // Removed duplicate definition of isLoading$

  // Removed duplicate implementation of getStatisticData

  // Removed duplicate implementation of getClusterGroupDetails

  // Removed duplicate declaration of assigneeList$
...
// =================================
  ...
  // Removed duplicate createCluster method to resolve errors

  // Removed duplicate definition of getSingleItemByBookId to resolve the error
