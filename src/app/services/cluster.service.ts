...
export class ClusterService {
 ...
 autoClusterListSubject$ = new BehaviorSubject<string[]>([]);
  ...
  async getAutoClusterData(currentTabKey?: string,params?: { [key: string]: string | number }) {
    try {
      console.log('Calling ClusterApiService.getAutoClusterData with params:', params);
      const res = (await this.#clusterApiService.getAutoClusterData(params))
        .pipe(
          take(1),
          tap((data: any) => {
            console.log('Fetched data:', data); // בדיקה שהנתונים מגיעים
            this.autoClusterListSubject$.next(data); 
            // שמירת הנתונים ב-BehaviorSubject
            if (currentTabKey && Array.isArray(data[currentTabKey])) {
              this.totalRows = data[currentTabKey].length;
              this.totalRowsSubject$.next(this.totalRows); 
              console.log(`Total rows for ${currentTabKey}:`, this.totalRows); // בדיקה של מספר השורות
              
            } else {
              console.warn('Could not determine totalRows for the current tab');
              this.totalRows = 0; 
            }
          }),
          catchError(err => {
